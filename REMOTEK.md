...
| `go.mod`, `go.sum` (dipendenze) | sei moduli alzati alla versione minima che corregge (`x/text`, `pgx/v5`, `go-jose/v4`, `logrus`, `x/net`, `jwt/v5`); `x/crypto`, `x/sys`, `x/sync`, `x/tools` salgono di conseguenza; nessun salto di major | chiudono le 6 vulnerabilita' chiamate dal codice trovate da govulncheck (elenco in `CHANGELOG.md`) | REGOLE 8, 9 |
